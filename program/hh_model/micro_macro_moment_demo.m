...
function data = simulate_dynare_micro(params, settings)
    dynare_file = fullfile(settings.base_dir, settings.mod_name);
    write_dynare_params(params, settings, settings.base_dir);
    addpath(settings.base_dir);
...
function write_dynare_params(params, settings, target_dir)
    param_file = fullfile(target_dir, 'micro_macro_params.m');
